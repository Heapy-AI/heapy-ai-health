...
대화 문맥 판단은 첫 질문에서만 실행하지 않는다. 최근 대화나 세션 요약이 있는 두 번째
질문부터는 규칙으로 선별하지 않고 항상 LLM에 `summary`, 최근 대화, 현재 질문을 전달한다.
LLM은 `standalone_question`, `is_follow_up`, `current_topic`, `inherited_target`,
`personal_context_required`를 구조화해 반환한다. 독립형 질문은 기존과 동일하게 의료용어
정규화와 Intent 분류·검색에 사용하며, 개인 건강검진 필요 판정은 RDB 조회 경로의 보조
신호로 사용한다. 문맥 판단 실패 시 원문과 기존 첫 질문 폴백으로 계속 처리한다.
...
이전 턴이 본인의 건강검진 결과를 사용한 대화라면, 후속 질문 재작성 결과가 사용자
이름을 포함하더라도 이전 본인 검진 문맥과 현재 검진 결과 참조를 함께 확인해 개인
컨텍스트 조회를 유지한다. 제3자 이름만 포함된 첫 질문에는 로그인 사용자의 검진값을
결합하지 않는다.
...
최종 생성 LLM에는 문맥 복원·정규화 질문과 함께 사용자 원문 질문 및 제한된 최근 대화도
전달한다. 원문과 최근 대화는 대명사·대상·말투를 이해하는 데만 사용하며 의료 사실은
계속 Pinecone 검색 청크와 인증된 개인 건강검진 RDB 컨텍스트에만 근거한다.
...
최종 답변 생성기는 현재 질문의 대상·요청 작업·설명 범위를 먼저 판단하고, 검색 청크와
개인 검진 컨텍스트 중 질문에 직접 필요한 근거만 선택한다. 검색 결과는 답변 가능한
근거의 모음으로 취급하며 전체 내용을 요약하지 않는다. 상태 평가에는 결론과 관련
측정값·DB 상태 및 최소 근거만 제시하고, 요청하지 않은 정의·원인·관리법 등으로 범위를
확장하지 않는다. 상세 요청이 없으면 가장 짧은 완전한 답변을 선택해 기본 2~4개의 짧은
문장으로 구성하고, 여러 항목의 나열이 필요한 경우에만 불릿을 사용한다.
