...
class LifestyleReportContent(BaseModel):
    ...
    #
    # 한때 current_state 아래에 key_points를 따로 뒀다. 없앴다가 한 번 되살려 봤는데,
    # 긴 문단 바로 밑에 그 문단을 간추린 목록이 붙으니 같은 말을 두 번 읽는 꼴이었다.
    # 설명이 이미 이어지게 쓰였다면 따로 간추릴 것이 없다. 그래서 다시 없앤다.
    # 상한이 3일 때는 데이터에 없는 일반 조언으로 세 번째를 채웠다. 그래서 둘로 줄였다.
