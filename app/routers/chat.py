...
PROGRESS_MESSAGES = {
    ...
    "classify_intent": (
        "질문의 유형과 안전 기준을 확인하는 중입니다"
    ),
    "load_health_context": (
        "관련 개인 건강 기록을 확인하는 중입니다"
    ),
    "search_evidence": (
        "관련 건강정보 근거를 찾는 중입니다"
    ),
...
class _CitationLabelStreamFilter:
    ...
    def feed(
        ...
        for character in text:
            ...
            if self._is_complete_label(
                self._pending
            ):
                self._pending = ""

            elif not self._is_partial_label(
                self._pending
            ):
...
def _to_answer_chunks(
    result: ChatOrchestrationResult,
) -> list[CombinedAnswerChunk]:
...
def chat(
    request: ChatRequest,
    session: AuthenticatedSession | None = Depends(
        optional_current_session
    ),
) -> ChatResponse:

    orchestrator = state.get(
        "chat_orchestrator"
    )
    ...
    try:
        ...
        result = orchestrator.answer(
            ...
            confirmation_answer=(
                request.confirmation_answer
            ),
            personal_context_loader=(
                _personal_context_loader(
                    session
                )
            ),
        )
    ...
    def generate_events() -> Iterator[str]:
        ...
        try:
            ...
            for stream_event in (
                ...
                if (
                    _should_persist_conversation_turn(
                        stream_event.result,
                        session,
                        session_id,
                    )
